...
当前范围更新（2026-08-31）：Skill 来源仅为 `~/.agents/skills`；移除设置窗口与引导页和菜单栏入口，启动显示常规的可缩放技能库窗口；技能库初始不展开元素或高亮列表行；Pin 按钮保留在左栏底部，表示窗口置顶。历史设置/引导/菜单栏故事不再属于当前 UI 交付范围，权限由用户手动管理。
